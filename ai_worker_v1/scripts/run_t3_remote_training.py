...
def patch_rfdetr_coco_extended_metrics() -> None:
    """Patch RF-DETR 1.3.0 COCO summary for numpy versions where argwhere is not scalar."""
    import rfdetr.engine as rfdetr_engine
    ...
    def safe_coco_extended_metrics(coco_eval: Any) -> dict[str, Any]:
        try:
            ...
            iou50_matches = np.flatnonzero(np.isclose(iou_thrs, 0.50))
            iou50_idx = int(iou50_matches[0]) if len(iou50_matches) else 0
            area_idx, maxdet_idx = 0, 2

            precision_tensor = coco_eval.eval["precision"]
            scores_tensor = coco_eval.eval["scores"]
            prec_raw = precision_tensor[iou50_idx, :, :, area_idx, maxdet_idx]
            prec = prec_raw.copy().astype(float)
            prec[prec < 0] = np.nan

            f1_cls = 2 * prec * rec_thrs[:, None] / (prec + rec_thrs[:, None])
            f1_macro = np.nanmean(f1_cls, axis=1)
            best_j = int(np.nanargmax(f1_macro)) if np.isfinite(f1_macro).any() else 0

            macro_precision = float(np.nanmean(prec[best_j])) if np.isfinite(prec[best_j]).any() else 0.0
            macro_recall = float(rec_thrs[best_j]) if len(rec_thrs) else 0.0

            score_vec = scores_tensor[iou50_idx, best_j, :, area_idx, maxdet_idx].astype(float)
            score_vec[prec_raw[best_j] < 0] = np.nan
            score_thr = float(np.nanmean(score_vec)) if np.isfinite(score_vec).any() else 0.0

            class_map = []
            cat_ids = coco_eval.params.catIds
            cat_id_to_name = {c["id"]: c["name"] for c in coco_eval.cocoGt.loadCats(cat_ids)}
            for k, cid in enumerate(cat_ids):
                p_slice = precision_tensor[:, :, k, area_idx, maxdet_idx]
                valid = p_slice > -1
                ap_50_95 = float(p_slice[valid].mean()) if valid.any() else float("nan")
                p50 = p_slice[iou50_idx]
                ap_50 = float(p50[p50 > -1].mean()) if (p50 > -1).any() else float("nan")
            ...
            map_50_95 = float(coco_eval.stats[0]) if len(coco_eval.stats) > 0 else 0.0
            map_50 = float(coco_eval.stats[1]) if len(coco_eval.stats) > 1 else 0.0
            class_map.append(
                {
                    "class": "all",
                    "map@50:95": map_50_95,
            ...
            return {"class_map": class_map, "map": map_50, "precision": macro_precision, "recall": macro_recall}
        except Exception as exc:
            stats = getattr(coco_eval, "stats", [])
            return {
                "class_map": [],
                "map": float(stats[1]) if len(stats) > 1 else 0.0,
                "precision": 0.0,
                "recall": 0.0,
                "patch_warning": f"coco_extended_metrics_fallback:{type(exc).__name__}:{exc}",
            }

    rfdetr_engine.coco_extended_metrics = safe_coco_extended_metrics
...
def temporal_eval(
    ...
    for row in seq:
        ...
        row_bbox = row.get("bbox_xyxy")
        gt = [row_bbox] if row.get("ground_truth") and isinstance(row_bbox, list) and len(row_bbox) == 4 else []
        preds = {
    ...
    for model_name, rows in per_model.items():
        ...
        if current:
            gaps.append(current)
        predicted_frames = sum(1 for r in rows if r["prediction_count"] > 0)
        metrics[model_name] = {
            "frames": len(rows),
            "visible_frames": len(visible),
            "visible_frame_recall": len(hits) / len(visible) if visible else 0.0,
            "precision": len(hits) / predicted_frames if predicted_frames else 0.0,
...
def main() -> int:
    ...
    if not (smoke_ckpts["last"] or smoke_ckpts["best"]):
        write_json(
            out_dir / "failure_report.json",
            {"status": "blocked_smoke_checkpoint_missing", "checkpoints": smoke_ckpts},
        )
        return 4
